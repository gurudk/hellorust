...
impl AsciiDraw {
    ...
    fn line(&mut self, x1: i32, y1: i32, x2: i32, y2: i32, c: char) -> &mut Self {
        if x1 == x2 {
            if y1 < y2 {
                for y in y1..y2 {
    ...
    fn circle(&mut self, x0: i32, y0: i32, r: i32, c: char) -> &mut Self {
        for x in x0 - r..=x0 + r {
            let xx = x - x0;
    ...
    fn draw_box(&mut self, x0: usize, y0: usize, text_width: usize, text: String) -> &mut Self {
        let len = text.chars().count();
        let mut rows = len / text_width;
        let r = len % text_width;
        if r > 0 {
